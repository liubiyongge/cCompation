...
{
	assert(data != NULL);
	SIMPLE_CONCURRENT_LIST_NODE *node = (SIMPLE_CONCURRENT_LIST_NODE *)malloc(sizeof(SIMPLE_CONCURRENT_LIST_NODE));
	node->marked_for_deletion = 0;
	node->data = data;
	node->next = NULL;
	if (list->size == 0) {
...
int mark_element_for_deletion_from_simple_concurrent_list(SIMPLE_CONCURRENT_LIST *list, void *data)
{
	...
	while (node != NULL) {
		if (node->data == data) {
			...
			SPDLOG_DEBUG("\t marked connection\n");
			return 1;
		}
		node = node->next;
	}
