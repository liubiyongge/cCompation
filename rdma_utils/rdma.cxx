...
#include "../utils/simple_concurrent_list.h"
#include "../utils/spin_loop.h"
...
msg_header *client_allocate_rdma_message(connection_rdma *conn, int message_payload_size, int message_type)
{
	...
	while (1) {
		...
		default:
			SPDLOG_ERROR("Unhandled state");
			_exit(EXIT_FAILURE);
		}

		if (stat == ALLOCATION_IS_SUCCESSFULL){
			break;
		}
	}
...
int __send_rdma_message(connection_rdma *conn, msg_header *msg, struct rdma_message_context *msg_ctx)
{
   ...
   * */
	...
	if (!msg_ctx && LIBRARY_MODE == SERVER_MODE) {
		...
		default:
			assert(0);
			SPDLOG_ERROR("Unhandled message type {}", msg->msg_type);
			_exit(EXIT_FAILURE);
		}
	} else {
...
struct ibv_context *get_rdma_device_context(char *devname)
{
	...
	if (num_devices < 1) {
		SPDLOG_ERROR("No RDMA device found. Exiting..");
		exit(EXIT_FAILURE);
	}

	if (!devname) {
		SPDLOG_INFO("Using default RDMA device %s", dev_list[0]->device->name);
		return dev_list[0];
	}
	...
	if (!rdma_dev) {
		SPDLOG_ERROR("Cannot find RDMA device {}", devname);
		_exit(EXIT_FAILURE);
	}
					     ...
					     struct channel_rdma *channel, connection_type type)
{
	...
	if (!strcmp(hosts[0], "127.0.0.1")) {
		SPDLOG_INFO("Connection with local host?");
		return;
	}
	...
	*colon = '\0';
	...
	SPDLOG_INFO("Connecting to %s at port %s", ip, port);
	int ret = rdma_getaddrinfo(ip, port, &hints, &res);
	if (ret) {
		SPDLOG_ERROR("rdma_getaddrinfo: {}, {}\n", hosts[0], strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if (ret) {
		SPDLOG_ERROR("rdma_create_ep: %s", strerror(errno));
		/*raise(SIGINT);*/
		exit(EXIT_FAILURE);
	}
	...
	while (tries < 100) {
		...
		if (ret) {
			SPDLOG_INFO("rdma_connect failed reconnecting: %s", strerror(errno));
			usleep(50000);
			++tries;
		} else
			break;
	...
	if (ret) {
		SPDLOG_ERROR("rdma_connect failed: {}", strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if (ret) {
		SPDLOG_ERROR("rdma_post_recv: {}", strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if (ret) {
		SPDLOG_ERROR("rdma_post_send: {}", strerror(errno));
		exit(EXIT_FAILURE);
	}
	/*free(host_copy);*/
	switch (type) {
	case MASTER_TO_REPLICA_CONNECTION:
		SPDLOG_DEBUG("Remote side accepted created a new MASTER_TO_REPLICA_CONNECTION");
	...
	case SERVER_TO_CLIENT_CONNECTION:
		SPDLOG_WARN("Should not handle this kind of connection here");
		break;
	default:
		SPDLOG_ERROR("BAD connection type");
		exit(EXIT_FAILURE);
	}
	...
	if (sem_init(&conn->congestion_control, 0, 0) != 0) {
		SPDLOG_ERROR("failed to initialize semaphore reason follows");
		perror("Reason: ");
	}
	...
	switch (conn->type) {
	case CLIENT_TO_SERVER_CONNECTION:
		SPDLOG_DEBUG("Initializing client communication circular buffer");
	...
	case MASTER_TO_REPLICA_CONNECTION:
		SPDLOG_DEBUG("Initializing master to replica communication circular buffer");
		conn->send_circular_buf =
			create_and_init_circular_buffer(conn->rdma_memory_regions->local_memory_buffer,
...
void crdma_init_generic_create_channel(struct channel_rdma *channel, char *ib_devname)
{
	...
	if (channel->comp_channel == 0) {
		SPDLOG_ERROR("building context reason follows:");
		perror("Reason: \n");
		_exit(EXIT_FAILURE);
	}
	...
	if (pthread_create(&channel->cq_poller_thread, NULL, poll_cq, channel) != 0) {
		SPDLOG_ERROR("Failed to create poll_cq thread reason follows:");
		perror("Reason: \n");
		_exit(EXIT_FAILURE);
	}
}
...
struct channel_rdma *crdma_client_create_channel(char *ib_devname)
{
	...
	channel = (channel_rdma *) malloc(sizeof(*channel));
							   ...
							   connection_type type)
{
	...
	if (conn == NULL) {
		SPDLOG_ERROR("FATAL ERROR malloc failed\n");
		exit(EXIT_FAILURE);
	}
...
void update_rendezvous_location(connection_rdma *conn, uint32_t message_size)
{
	...
	if (conn->type == SERVER_TO_CLIENT_CONNECTION || conn->type == REPLICA_TO_MASTER_CONNECTION) {
		...
		SPDLOG_ERROR("Faulty connection type");
		_exit(EXIT_FAILURE);
	}
}
...
void close_and_free_RDMA_connection(struct channel_rdma *channel, struct connection_rdma *conn)
{
	...
	SPDLOG_DEBUG("\t * Removed connection form list successfully :-) \n");
	channel->nconn--;
	channel->nused--;
	...
	free(conn);
	SPDLOG_DEBUG("\t*Destroyed RDMA connection successfully\n");
}
...
static void error_to_string(int error)
{
	...
	case IBV_WC_REM_ACCESS_ERR:
		SPDLOG_ERROR("Remote memory error");
		break;
	case IBV_WC_LOC_ACCESS_ERR:
		SPDLOG_ERROR("Local memory error");
		break;
	default:
		SPDLOG_ERROR("Unknown error code");
	}
}
...
void *poll_cq(void *arg)
{
	...
	while (1) {
		if (ibv_get_cq_event(channel->comp_channel, &cq, &ev_ctx) != 0) {
			SPDLOG_ERROR("polling cq failure reason follows");
			perror("Reason: \n");
			_exit(EXIT_FAILURE);
		}
		...
		while (1) {
			...
			if (rc < 0) {
				SPDLOG_ERROR("poll of completion queue failed!");
				exit(EXIT_FAILURE);
			} else if (rc > 0) {
		...
		case IBV_WC_RDMA_WRITE:
			...
			if (msg) {
				...
				case DISCONNECT:
					break;
				default:
					SPDLOG_ERROR("Entered unplanned state FATAL for message type {}", msg->msg_type);
					_exit(EXIT_FAILURE);
				}
			}
			break;
		case IBV_WC_RDMA_READ:
			SPDLOG_DEBUG("IBV_WC_RDMA_READ code");
			break;
		case IBV_WC_COMP_SWAP:
			SPDLOG_DEBUG("IBV_WC_COMP_SWAP code");
			break;
		case IBV_WC_FETCH_ADD:
			SPDLOG_DEBUG("IBV_WC_FETCH_ADD code");
			break;
		case IBV_WC_BIND_MW:
			SPDLOG_DEBUG("IBV_WC_BIND_MW code");
			break;
		case IBV_WC_RECV_RDMA_WITH_IMM:
			SPDLOG_DEBUG("IBV_WC_RECV_RDMA_WITH_IMM code");
			break;
		default:
			SPDLOG_ERROR("FATAL unknown code");
			/*exit(EXIT_FAILURE);*/
		}
	} else {
		/*error handling*/
		SPDLOG_ERROR("conn type is {} {}\n", conn->type, ibv_wc_status_str(wc->status));
