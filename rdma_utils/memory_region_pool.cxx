				  ...
				  size_t allocation_size)
{
	...
	if (!pool) {
		SPDLOG_ERROR("Allocation of new memory region pool failed\n");
		return NULL;
	}
...
memory_region *mrpool_allocate_memory_region(memory_region_pool *pool, struct rdma_cm_id *id)
{
   ...
   */
	...
	if (pool->type == PREALLOCATED) {
		...
		struct klist_node *freelist_node = (struct klist_node *)klist_get_first(freelist);
		if (freelist_node) {
		...
		if (!mr) {
			SPDLOG_WARN("Allocation of new memory region failed\n");
			return NULL;
		}
...
void mrpool_free_memory_region(memory_region **mr)
{
	...
	if (pool->type == PREALLOCATED) {
		...
		if (rdma_dereg_mr((*mr)->local_memory_region)) {
			SPDLOG_ERROR("ibv_dereg_mr failed: {}\n", strerror(errno));
		}
		if (rdma_dereg_mr((*mr)->remote_memory_region)) {
			SPDLOG_ERROR("ERROR: ibv_dereg_mr failed: {}\n", strerror(errno));
		}
#if ALLOC_LOCAL
...
static int _mrpool_preallocate_mr(memory_region_pool *pool)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (!mr) {
			SPDLOG_ERROR("Preallocation of new memory region failed\n");
			return 1;
		}
...
#if ALLOC_LOCAL
	if ((mr->local_memory_buffer = (char *)numa_alloc_local(mr->memory_region_length)) == NULL) {
		SPDLOG_ERROR("FATAL Allocation for local memory region failed\n");
		exit(EXIT_FAILURE);
	}
#else
...
#if ALLOC_LOCAL
	if ((mr->remote_memory_buffer = (char *)numa_alloc_local(mr->memory_region_length)) == NULL) {
		SPDLOG_ERROR("FATAL Allocation for remote memory region failed\n");
		exit(EXIT_FAILURE);
	}
#else
